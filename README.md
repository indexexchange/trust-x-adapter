...
* `trust-x-htb.js` - This is your partner module file, by default it contains a template divided into multiple sections which need to be completed.
* `trust-x-htb-validator.js` - This is the validator file for the configuration object that will be passed into your module.
* `trust-x-htb-exports.js` - A file that contains all of the modules exports (i.e. any functions that need to be exposed to the outside world).
...
1. <b>Complete the trust-x-htb.js file </b>
    * trust-x-htb.js is where all of your adapter code will live.
...
2. <b>Complete the trust-x-htb-validator.js file</b>
...
3. <b>Complete the trust-x-htb-exports.js file</b>
    * This file will contain any functions that need to be exported or exposed to the outside world. Things like render functions, custom callbacks, etc. Any legacy render functions will also need to be exposed here. Anything added to the `shellInterface.TrustXHtb` will be accessible through `window.headertag.TrustXHtb`
...
In this section you will be filling out the trust-x-htb.js, trust-x-htb-exports.js, and the trust-x-htb-validator.js files to create your module.

### Step 0: Config Validation (`trust-x-htb-validator.js`)
...
Once you have a basic idea of what this will look like, and how you will uniquely identify each slot on your server (via xSlot placementId or other inventory codes) you will need to validate this configuration. This validation will be performed by the wrapper using the `trust-x-htb-validator.js` file.

The `trust-x-htb-validator.js` file contains a single export, a `partnerValidator` function, that takes in the configuration object that will be fed to your module's constructor (refer to [Configuration](#configuration) for an example layout) and validates it via type checks. The type checks are performed using an external library called `schema-inspector`, for which the documentation can be found here https://github.com/Atinux/schema-inspector.

We have provided a very basic validation schema in `trust-x-htb-validator.js` that is based off of the example `mockPartnerConfig.js` object found in the `spec/support` directory for testing (refer to the [Testing](#testing) section for the testing structure).

Once you have filled this file out, you can continue actually writing your module!

### Step 1: Partner Configuration (`trust-x-htb.js`)
...
### Step 2: Generate Request URL (`trust-x-htb.js`)
...
### Step 3: Response Callback (`trust-x-htb.js`)
...
### Step 4: Parsing and Storing Demand (`trust-x-htb.js`)
...
```javascript
{
    ...
    "targetingType": "slot"
    "targeting" : {
        "ix_TRSTX_id": ["_230l09jd2"],
        "ix_TRSTX_cpm": ["300x250_2.50"]
    },
    "price": 2.50,
    "size": [300,250],
...
### Step 5: Rendering Pixel (`trust-x-htb.js`)
...
### Step 6: Exports (`trust-x-htb-exports.js`)
...
```javascript
shellInterface.TrustXHtb = { //shell interface is the window variable that is accessable through the window object, currently this will always be window.headertag
...
if (__directInterface.Layers.PartnersLayer.Partners.TrustXHtb) {
    ...
    shellInterface.TrustXHtb.adResponseCallback = __directInterface.Layers.PartnersLayer.Partners.TrustXHtb.adResponseCallback;
}
```
